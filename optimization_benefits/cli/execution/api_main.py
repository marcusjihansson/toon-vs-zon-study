#!/usr/bin/env python3
"""
main.py - RAG from Shopify API

This module demonstrates RAG (Retrieval Augmented Generation) using products
fetched directly from the Shopify API. It benchmarks different serialization
adapters (JSON, TOON, BAML, ZON) to compare token efficiency and performance.

Usage:
    python cli/api.py

This runs the complete benchmark comparing all strategies on API data.
"""
...
class ShopifyAPIRAG:
    """RAG system that fetches products directly from Shopify API."""
    ...
    def prepare_context(self, products: List[Dict[str, Any]], query: str) -> str:
        ...
        """
        from adapters.serializers import (serialize_baml, serialize_combined,
                                          serialize_json, serialize_toon,
                                          serialize_zon)
        # Note: serialize_zon added for strict strategy support
        from strategies import (BAMLStrategy, BaselineStrategy, BaseStrategy,
                                CombinedStrategy, JSONStrategy, ToonStrategy,
                                ToonStrictStrategy, ZONCombinedStrategy,
                                ZONStrictStrategy)

        strategy_name = (
            self.strategy.name if hasattr(self.strategy, "name") else "baseline"
        )
        ...
        context = f"""Product Catalog (fetched from Shopify API in {format_name} format):
...
def run_api_benchmark(queries: List[str]) -> Dict[str, Any] | None:
    """Run benchmark comparing all strategies on API data.
    ...
    """
    print("\n" + "=" * 70)
    print("SHOPIFY API RAG BENCHMARK")
    print("=" * 70)
...
def compare_token_efficiency(query: str = "What products are available?"):
    """Compare token efficiency between strategies on API data.
    ...
    print("\n" + "=" * 70)
    print("TOKEN EFFICIENCY COMPARISON (Shopify API)")
    print("=" * 70)
    print(f"Query: {query}")
...
def main():
    """Main entry point."""
    print("\n" + "=" * 70)
    print("SHOPIFY API RAG - Strategy Benchmarking")
    print("=" * 70)
